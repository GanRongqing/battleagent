...
class LLMCommander:
    ...
    def maybe_request(self, now_sim, summary_text, valid_tracks, trigger="periodic",
        ...
        t = threading.Thread(target=self._worker,
                             args=(now_sim, summary_text, list(valid_tracks), trigger, ctx),
                             daemon=True)
        t.start()
        return True

    def _worker(self, now_sim, summary_text, valid_tracks, trigger, ctx):
        t0 = time.time()
...
class AgentMain:
    ...
    def run(self):
        print("=" * 72)
        print("HYBRID AGENT V4 — Deterministic Harness + Bounded DeepSeek Commander")
