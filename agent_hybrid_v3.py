...
class StrategicIntent:
    ...
    __slots__ = ("posture", "focus_level", "emergency_focus_level", "reserve_usvs",
                 ...
                 "standoff_preference", "uncertainty_tolerance", "reason")
    ...
    def __init__(self, posture="balanced", focus_level=2, emergency_focus_level=3,
                 ...
                 standoff_preference="medium", uncertainty_tolerance=0.35, reason=""):
...
class TacticalSummarizer:
    ...
    def build(self, obs, tracker, usv_ctrl, uav_mgr, intent):
        ...
        n_air = sum(1 for v in uav_st.values() if v in (UAVManager.SEARCH, UAVManager.SCREEN, UAVManager.REACQUIRE))
...
class ThreatAllocator:
    ...
    def allocate_usvs(self, tracks, usvs, usv_map, now, intent=None):
        ...
        def commit_guard(t):
            if t.is_visible(now):
                return False
            if t.point_confidence < self.LOW_CONF_GUARD:
                pos = t.predicted_position(now)
        ...
        def target_value(name, t):
            ...
            if not t.is_visible(now) and t.point_confidence < self.LOW_CONF_GUARD:
                pos = t.predicted_position(now)
                if pos is not None and pos[0] >= self.URGENT_X:
                    mg *= 0.35
            return base * mg
        ...
        def _assign_one(target_name, target_pos):
            ...
            if not tracks[target_name].is_visible(now) and \
                    tracks[target_name].point_confidence < self.LOW_CONF_GUARD:
        ...
        while available:
            ...
            if best_name is None:
                ...
                if not active_engagement and guarded:
                    ...
                    if self.threat_score(t, now, intent=intent) * self.marginal_gain(len(t.assigned_usvs)) >= self.MIN_ASSIGN_VALUE:
                        best_name, best_val = name, 0.0
            if best_name is None or best_val < self.MIN_ASSIGN_VALUE:
                break
            t = tracks[best_name]
            tpos = t.predicted_position(now)
...
class USVController:
    ...
    def step(self, obs, tracks, legal, alloc_result, events, intent=None):
...
class UAVManager:
    ...
    ON_SHIP, SEARCH, SCREEN, REACQUIRE, RETURN, LANDING, CHARGING, SAFE_LOITER = \
        "ON_SHIP", "SEARCH", "SCREEN", "REACQUIRE", "RETURN", "LANDING", "CHARGING", "SAFE_LOITER"
    ...
    def step(self, obs, tracker, legal, events, intent=None, threat_fn=None):
        ...
        for u in obs.uavs:
            ...
            # ---- SEARCH / SCREEN / REACQUIRE（动态角色分配）----
            if st in (self.SEARCH, self.SCREEN, self.REACQUIRE):
                ...
                if scr is not None and recon >= 0.45:
...
class AgentMain:
    ...
    def run(self):
        print("=" * 72)
        print("HYBRID AGENT V3 — Maneuver-aware Belief + Dynamic Sensing + "
              "Marginal Allocation + LLM Commander")
