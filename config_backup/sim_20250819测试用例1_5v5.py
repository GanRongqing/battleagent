...
def sim(simserver=None, engine_name='20250803水面所仿真2', web_ip=None, gengtu_ip=None, user_name='admin',
    ...
    # 泛化: 每次对局从Demo方案中随机选择一个黑方策略
    _demo_dir = "/root/autodl-tmp/hsystem/hsystem/sim_script/20250819TZB/Demo"
    _plan_files = sorted(_glob.glob(_os.path.join(_demo_dir, "方案*.json")))
    assert _plan_files, "Demo目录下没有黑方方案JSON"
    _chosen_plan = _random.choice(_plan_files)
    print(f"[泛化] 本局黑方方案: {_os.path.basename(_chosen_plan)}")
    with open(_chosen_plan, 'r', encoding='utf-8') as f2:
        black_waypoints = json.load(f2)

    engine.cache["num_black_usv"] = int(black_waypoints.get("usvNum", 5))
    ...
    # 我方兵力固定5艘USV+5架UAV, 初始位置默认(集中在x=0附近)
    ships_xy = {"white_usv1":[0, 345000], "white_usv2":[0, 340000], "white_usv3":[0, 335000], "white_usv4":[0, 330000], "white_usv5":[0, 325000]}
    uav_swarm_xy = {"white_uav1":[0, 345000], "white_uav2":[0, 340000], "white_uav3":[0, 335000], "white_uav4":[0, 330000], "white_uav5":[0, 325000]}
    ...
    # 动态生成黑方艇(数量随随机方案变化)
    blues = []  # 所有蓝方单位
    for _bi in range(1, engine.cache["num_black_usv"] + 1):
        _key = f"usv{_bi}"
        if _key not in black_waypoints:
            break
        _b = engine.gen_platform(f'black_usv{_bi}', 'Ship', 'BLUE',
                                 black_waypoints[_key][0]["point"],
                                 black_waypoints[_key][0]["speed"],
                                 0, coordinate_system="Cartesian")
        blues.append(_b)
        # 为每艘黑方USV设置航行区域（按航路点巡逻）
        engine.cmd_sail_area(_b, speed=10, xy_points=[j['point'] for j in black_waypoints[_key]])
